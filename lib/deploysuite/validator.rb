module Deploysuite
	class Validator
		...
		def get_git_branch(machine_name, ymlfiles_path)
			# ymlfiles_yml = '/rails/.deploysuite/rails_ymlfiles.yml'
			machines = get_machines_at_deploy_levels(ymlfiles_path)

			if machines[:dev_machines].include?(machine_name)
				return 'dev'
			elsif machines[:qa_machines].include?(machine_name)
				return 'qa'
			elsif machines[:prod_machines].include?(machine_name)
				return 'master'
			else
		...
		def get_machine_deployment_level(machine_name, ymlfiles_path)
			# ymlfiles_yml = '/rails/.deploysuite/rails_ymlfiles.yml'
			machines = get_machines_at_deploy_levels(ymlfiles_path)
